...
[Authorize(Roles = "Administrador,RRHH,TI")]
public class ExpedientesTIController : Controller
{
    ...
    public ExpedientesTIController(Farmacol1Context context,
        IWebHostEnvironment env,
        DocumentoService docSvc,
        AuditService audit,
        ExcelService excel)
    {
        _context = context; _env = env; _docSvc = docSvc; _audit = audit;
        _excel = excel;
    }
    ...
    public async Task<IActionResult> Index(string? busqueda, string? area)
    {
        ...
        var conteo = (await _context.TbExpedientes
            .Where(d => ccs.Contains(d.CC) && d.Modulo == "TI").Select(d => d.CC).ToListAsync())
            .GroupBy(cc => cc).ToDictionary(g => g.Key, g => g.Count());

        var docsRecientes = (await _context.TbExpedientes
            .Where(d => ccs.Contains(d.CC) && d.Modulo == "TI")
            .OrderByDescending(d => d.FechaSubida).ToListAsync())
            .GroupBy(d => d.CC).ToDictionary(g => g.Key, g => g.Take(3).ToList());

        var equiposPorCC = (await _context.Tbinventarios
            .Where(e => e.CC != null && ccs.Contains(e.CC.Value)).ToListAsync())
            .GroupBy(e => e.CC!.Value).ToDictionary(g => g.Key, g => g.ToList());
        ...
        ViewBag.Conteo = conteo; ViewBag.Personal = personal;
        ViewBag.DocsRecientes = docsRecientes; ViewBag.EquiposPorCC = equiposPorCC;
        ViewBag.Plantillas = await _context.TbPlantillas
                               .Where(p => p.Modulo == "TI" && p.Activa).ToListAsync();
        return View();
    }

    [HttpGet]
    public async Task<IActionResult> Previo(int cc, string tipoDocumento)
    {
        ...
        var plantilla = await _context.TbPlantillas
            .FirstOrDefaultAsync(p => p.TipoDocumento == tipoDocumento && p.Modulo == "TI" && p.Activa);
        if (plantilla == null) { TempData["Error"] = $"No hay plantilla para '{tipoDocumento}'."; return RedirectToAction(nameof(Index)); }
        var equipos = await _context.Tbinventarios.Where(e => e.CC == cc).ToListAsync();
        ViewBag.Personal = personal; ViewBag.TipoDocumento = tipoDocumento; ViewBag.Equipos = equipos;
        return View();
    }
    ...
    public async Task<IActionResult> Generar(int cc, string tipoDocumento,
        [FromForm] Dictionary<int, string> observaciones)
    {
        var personal = await _context.Tbpersonals.FindAsync(cc);
        if (personal == null) return NotFound();
        var plantilla = await _context.TbPlantillas
        ...
        var docxBytes = await _docSvc.GenerarDocxAsync(rutaFisica, personal, equipos,
            observaciones, creadorUserName: User.Identity?.Name);
        var nombreBase = $"{tipoDocumento.Replace(" ", "_")}_{personal.CC}_{DateTime.Now:yyyyMMdd}";
        ...
        string? tipoDocumento, string? nombrePersonalizado)
    {
        ...
        try
        {
            ...
            $"Documento '{nombreDoc}' subido al expediente CC:{cc} (TI)", cc.ToString());
        }
        catch { }
        ...
        string? tipoDocumento, string? nombrePersonalizado)
    {
        ...
        {
            await _audit.RegistrarAsync(AuditService.MOD_EXPEDIENTES, AuditService.ACC_SUBIR,
            $"Subida masiva '{nombreDoc}' a {lista.Count} empleados (TI)");
        }
        catch { }
    ...
    public async Task<IActionResult> Eliminar(int id)
    {
        ...
        try
        {
            ...
            $"Documento '{doc.NombreArchivo}' eliminado del expediente CC:{cc} (TI)",
            id.ToString());
        }
        catch { }
    ...
    {
        var carpeta = Path.Combine(_env.WebRootPath, "expedientes", "ti", cc.ToString());
        ...
        return $"/expedientes/ti/{cc}/{nombre}";
    }
    ...
    {
        IQueryable<TbExpediente> query = _context.TbExpedientes.Where(d => d.Modulo == "TI");
        if (!todo)
        {
            if (!string.IsNullOrEmpty(busqueda))
        ...
        return File(bytes, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            $"ExpedientesTI_{DateTime.Now:yyyyMMdd_HHmm}.xlsx");
    }
}
